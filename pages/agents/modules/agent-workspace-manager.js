class AgentWorkspaceManager {
  ...
  // 智能体功能说明
  getAgentDescription(agentKey) {
    const descriptions = {
      hotspot: "🔥 热点追踪：实时追踪热门话题，为您推荐最适合的创作方向",
      script: "📝 脚本创作：根据热点主题，自动生成吸引人的短视频脚本",
      storyboard: "🎨 分镜设计：基于脚本内容，创作精美的分镜图片",
      video: "🎥 视频合成：将分镜图片合成为完整的短视频作品",
      editor: "✂️ 剪辑优化：对视频进行智能剪辑和优化处理",
      dataAnalysis: "📊 数据分析：分析作品数据，提供优化建议",
      roleConsistency: "👥 角色一致性：确保视频中的角色形象统一连贯",
      trendHunter: "🔥 热点追踪：实时追踪热门话题，为您推荐最适合的创作方向",
      scriptWriter: "📝 脚本创作：根据热点主题，自动生成吸引人的短视频脚本",
      storyboard: "🎨 分镜设计：基于脚本内容，创作精美的分镜图片",
      videoComposer: "🎥 视频合成：将分镜图片合成为完整的短视频作品",
      qualityChecker: "✂️ 剪辑优化：对视频进行智能剪辑和优化处理",
      platformAdapter: "📊 数据分析：分析作品数据，提供优化建议",
      autoPublisher: "👥 角色一致性：确保视频中的角色形象统一连贯",
    };
    return descriptions[agentKey] || `${agent.icon} ${agent.name}`;
  }
  ...
  onAgentTap(e) {
    ...
    if (agent.status === "working" || agent.status === "completed" || agent.output) {
      ...
      // 显示智能体功能说明
      const description = this.getAgentDescription(agentKey);
      wx.showModal({
