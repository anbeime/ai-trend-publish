...
Page({
  data: {
    ...
    // MiniMax TokenPlanPlus API配置（优先级最高，国内版）
    minimaxConfig: {
      enabled: true,
      apiKey: (() => {
        try {
      ...
      groupId: (() => {
        try {
          const secrets = require('../../config/secrets.js');
          return secrets.minimax?.groupId || "";
        } catch (e) {
          return "";
        }
      })(),
    ...
    // 可用的 GLM 模型列表
    glmModelOptions: [
      { value: "glm-4.7-flash", label: "GLM-4.7-Flash (推荐)" },
      { value: "glm-4.5-air", label: "GLM-4.5-Air (轻量)" },
      { value: "glm-4-flash", label: "GLM-4-Flash (标准)" },
    ],
    glmModelIndex: 0, // 默认选中第一个模型

    // 混元AI配置（用于内容生成的模型降级）
    hunyuanConfig: {
      enabled: true,              // 是否启用混元作为备用模型
      useAsPrimary: false,        // 是否优先使用混元（true=混元优先，false=GLM优先）
      model: "hunyuan-lite",      // 混元模型名称
      supportedAgents: ["content-creator"], // 支持的智能体类型
    },
  ...
  onLoad(options) {
    ...
    try {
      ...
      const testHotspots = [
        {
          id: 1,
          title: "AI绘画工具Midjourney推出V6版本，生成效果大幅提升",
          name: "AI绘画工具Midjourney推出V6版本，生成效果大幅提升",
          source: "科技媒体",
          hotness: 95,
          heat: 95,
          suitability: 88,
          tags: ["AI绘画", "Midjourney", "人工智能", "创意工具"]
        },
        {
          id: 2,
          title: "小红书爆款笔记创作秘籍：如何打造高互动内容",
          name: "小红书爆款笔记创作秘籍：如何打造高互动内容",
          source: "社交媒体",
          hotness: 87,
          heat: 87,
          suitability: 92,
          tags: ["小红书", "内容创作", "社交媒体", "爆款"]
        },
        {
          id: 3,
          title: "2024年短视频趋势分析：竖屏内容成主流",
          name: "2024年短视频趋势分析：竖屏内容成主流",
          source: "行业报告",
          hotness: 78,
          heat: 78,
          suitability: 85,
          tags: ["短视频", "趋势分析", "内容营销", "抖音"]
        },
        {
          id: 4,
          title: "微信公众号排版新技巧：提升阅读体验的5个方法",
          name: "微信公众号排版新技巧：提升阅读体验的5个方法",
          source: "自媒体",
          hotness: 72,
          heat: 72,
          suitability: 90,
          tags: ["微信公众号", "排版技巧", "阅读体验", "内容优化"]
        },
        {
          id: 5,
          title: "ChatGPT应用场景拓展：从聊天到内容创作全流程",
          name: "ChatGPT应用场景拓展：从聊天到内容创作全流程",
          source: "AI技术",
          hotness: 85,
          heat: 85,
          suitability: 82,
          tags: ["ChatGPT", "AI应用", "内容创作", "自动化"]
        }
      ];

      // 设置测试数据
      this.setData({
        realtimeHotspots: testHotspots,
        displayedHotspots: testHotspots
      });
  ...
  loadHunyuanConfig() {
    try {
      ...
      const hunyuanConfig = {
        ...
        useAsPrimary: config.useAsPrimary !== undefined ? config.useAsPrimary : this.data.hunyuanConfig.useAsPrimary,
      ...
      console.log("✅ 混元配置加载成功:", {
        enabled: hunyuanConfig.enabled,
        useAsPrimary: hunyuanConfig.useAsPrimary,
        model: hunyuanConfig.model,
      });
    } catch (error) {
  ...
  saveHunyuanConfig(config) {
    try {
      ...
      console.log("✅ 混元配置已保存:", {
        ...
        useAsPrimary: config.useAsPrimary,
        model: config.model,
      });
      ...
      if (
        ...
        if (validHotspots.length > 0) {
          ...
          if (category !== "all") {
            ...
            displayedHotspots = validHotspots.filter((item) =>
              validCategories.includes(item.category),
            );
            console.log(
              `📂 分类筛选: ${category} (匹配: ${validCategories.join(", ")}), 筛选出 ${displayedHotspots.length} 条`,
            );
          }
      ...
      if (result.success) {
        ...
        if (hotspots.length > 0) {
          ...
          if (category !== "all") {
            ...
            const validCategories = categoryMapping[category] || [category];
            displayedHotspots = hotspots.filter((item) =>
              validCategories.includes(item.category),
            );
            console.log(
              `📂 后台分类筛选: ${category} (匹配: ${validCategories.join(", ")}), 筛选出 ${displayedHotspots.length} 条`,
            );
          }
        ...
        console.warn("⚠️ 热点API返回数据格式异常:", result);
      }
    } catch (error) {
  ...
  async loadHotspots() {
    ...
    while (retryCount < maxRetries) {
      try {
        console.log(`🔄 开始获取热点数据 (尝试 ${retryCount + 1}/${maxRetries})`);

        const aiService = require('../../utils/ai-service.js');
        const result = await aiService.fetchHotspots();

        console.log("✅ 热点API调用成功:", result);

        if (!result.success) {
          throw new Error(result.error || "获取热点失败");
        }

        let hotspots = result.data || [];

        console.log(`✅ 获取到 ${hotspots.length} 条热点`);

        // 显示数据来源
        console.log(`📊 数据来源: 最新数据`);
        if (result.timestamp) {
        ...
        retryCount++;
        lastError = error;
        console.error(`❌ 加载热点失败 (尝试 ${retryCount}/${maxRetries}):`, error);
        console.error("错误详情:", {
          message: error.message,
          errMsg: error.errMsg,
          errCode: error.errCode,
        ...
        // 如果是 -404006 错误，等待后重试
        if (error.errMsg && error.errMsg.includes('-404006')) {
          console.log("检测到云函数响应超时，等待后重试...");
    ...
    // 所有重试都失败后的处理
    if (retryCount >= maxRetries || (lastError && !lastError.errMsg?.includes('-404006'))) {
      // 加载失败时，检查是否有缓存可用
      const hasCache = this.loadHotspotsFromStorage();
      ...
      // 判断是否是云函数部署问题
      const isCloudFunctionError = lastError?.errMsg?.includes('-404006') || 
                                    lastError?.errMsg?.includes('not found') ||
                                    lastError?.errCode === -1;

      const errorMessage = isCloudFunctionError 
        ? "云函数未部署或响应超时，请检查：\n1. 云函数是否已上传部署\n2. 在微信开发者工具中右键 cloudfunctions/hotspot-miyucaicai 选择「上传并部署」"
        : `错误信息: ${lastError?.message || lastError?.errMsg || "未知错误"}`;
  ...
  selectCategory(e) {
    try {
      ...
      const categoryMapping = {
        tech: ["tech", "科技"],
        life: ["life", "生活", "general"], // 兼容general分类
        entertainment: ["entertainment", "娱乐"],
        food: ["food", "美食"],
        travel: ["travel", "旅行"],
        finance: ["finance", "财经"], // 新增财经分类
        gaming: ["gaming", "游戏"], // 新增游戏分类
        sports: ["sports", "体育"], // 新增体育分类
      };
  ...
  onGLMModelChange(e) {
    ...
    const selectedModel = this.data.glmModelOptions[index];

    if (selectedModel) {
      // 更新 glmConfig 中的 model 字段
      this.setData({
        glmModelIndex: index,
        'glmConfig.model': selectedModel.value,
      });

      console.log("✅ 切换 GLM 模型:", selectedModel.value, selectedModel.label);

      wx.showToast({
        title: `已切换到 ${selectedModel.label}`,
        icon: 'none',
        duration: 1500,
      });
    }
  ...
  async doGenerateContent() {
    ...
    try {
      ...
      // 根据配置选择模型策略：支持 MiniMax / GLM / 混元 三模型降级
      // 优先级：MiniMax M2.7 > GLM > 混元
      const minimaxConfig = this.data.minimaxConfig || { enabled: true };
      const hunyuanConfig = this.data.hunyuanConfig || { enabled: true, useAsPrimary: false, model: "hunyuan-lite" };
      console.log("📋 当前模型策略配置:", {
        minimaxEnabled: minimaxConfig.enabled,
        hunyuanEnabled: hunyuanConfig.enabled,
        hunyuanUseAsPrimary: hunyuanConfig.useAsPrimary,
        hunyuanModel: hunyuanConfig.model,
      });
      let content = null;

      if (minimaxConfig.enabled) {
        // 策略1：MiniMax优先，GLM降级，混元兜底（推荐）
        console.log("🚀 使用MiniMax M2.7优先策略");
        this.setData({ loadingText: "✍️ MiniMax AI正在创作内容..." });
        try {
          content = await this.callMiniMaxAPI(prompt);
          console.log("✅ MiniMax内容生成成功:", content ? "有内容" : "无内容");
        } catch (minimaxError) {
          console.warn("⚠️ MiniMax模型失败，降级到GLM:", minimaxError.message);
          this.setData({ loadingText: "✍️ 切换到GLM模型创作中..." });
          wx.showToast({
            title: "MiniMax限流，切换GLM...",
            icon: 'none',
            duration: 2000,
          });
          try {
            content = await this.callGLMAPI(prompt);
            console.log("✅ GLM降级内容生成成功:", content ? "有内容" : "无内容");
          } catch (glmError) {
            if (hunyuanConfig.enabled) {
              console.warn("⚠️⚠️ GLM也失败，最后尝试混元降级!!!");
              this.setData({ loadingText: "⚡ 切换到混元AI..." });
              try {
                content = await this.callHunyuanAPI(prompt);
                console.log("✅ 混元兜底内容生成成功!!!");
              } catch (fallbackError) {
                console.error("❌ 所有模型均失败:", fallbackError.message);
                throw new Error(`内容生成失败 - MiniMax: ${minimaxError.message}, GLM: ${glmError.message}, 混元: ${fallbackError.message}`);
              }
            } else {
              throw new Error(`内容生成失败 - MiniMax: ${minimaxError.message}, GLM: ${glmError.message}`);
            }
          }
        }
      } else if (hunyuanConfig.useAsPrimary && hunyuanConfig.enabled) {
        // 策略2：混元优先，GLM降级
        console.log("🤖 使用混元模型优先策略");
        this.setData({ loadingText: "✍️ 混元AI正在创作内容..." });
        try {
          content = await this.callHunyuanAPI(prompt);
          console.log("✅ 混元模型内容生成成功");
        } catch (hunyuanError) {
          console.warn("⚠️ 混元模型失败，降级到GLM:", hunyuanError.message);
          this.setData({ loadingText: "✍️ 切换到GLM模型创作中..." });
          content = await this.callGLMAPI(prompt);
          console.log("✅ GLM降级内容生成成功");
        }
      } else if (hunyuanConfig.enabled) {
        // 策略3：GLM优先，混元降级
        console.log("🔄 使用GLM优先+混元降级策略");
        this.setData({ loadingText: "✍️ AI正在创作内容..." });
        try {
          content = await this.callGLMAPI(prompt);
          console.log("✅ GLM内容生成成功:", content ? "有内容" : "无内容");
        } catch (glmError) {
          console.warn("⚠️⚠️⚠️ GLM模型失败（可能限流），正在切换到混元模型!!! 原因:", glmError.message);
          this.setData({ loadingText: "⚡ GLM限流中，切换到混元AI..." });
          wx.showToast({
            title: "GLM限流，切换混元...",
            icon: 'none',
            duration: 2000,
          ...
          try {
            console.log("🤖🤖🤖 开始调用混元API降级!!!");
            content = await this.callHunyuanAPI(prompt);
            console.log("✅✅✅ 混元降级内容生成成功!!!");
          } catch (fallbackError) {
            console.error("❌❌❌ 混元降级也失败了:", fallbackError.message);
            throw new Error(`内容生成失败 - GLM: ${glmError.message}, 混元: ${fallbackError.message}`);
          }
        }
      } else {
        // 策略4：仅GLM（原始逻辑）
        console.log("📝 仅使用GLM模型");
        this.setData({ loadingText: "✍️ AI正在创作内容..." });
        content = await this.callGLMAPI(prompt);
        console.log("✅ 内容生成成功:", content ? "有内容" : "无内容");
      }

      console.log("✅ 内容生成完成:", content ? "有内容" : "无内容");
      ...
      // 处理 content 格式 - 使用 platformFormatter.parseBody 递归解析 JSON
      let contentText = content.content;
      
      console.log("🔧 原始 contentText 类型:", typeof contentText, "长度:", typeof contentText === 'string' ? contentText.length : 'N/A');
      
      // 直接使用 platformFormatter.parseBody 解析
      contentText = platformFormatter.parseBody(contentText);
      
      // 持续解析直到不再是 JSON
      let maxIter = 20;
      let iter = 0;
      while ((contentText.trim().startsWith('{') || contentText.trim().startsWith('[')) && iter < maxIter) {
        iter++;
        console.log(`🔧 第 ${iter} 次解析，contentText 仍是JSON格式`);
        const nextText = platformFormatter.parseBody(contentText);
        if (nextText === contentText) break;
        contentText = nextText;
      }
      
      console.log("🔧 最终提取的 contentText 长度:", contentText?.length, "类型:", typeof contentText);
      console.log("🔧 最终 contentText 前100字符:", contentText?.substring(0, 100));
      ...
      this.safeSetData(
        ...
        () => {
          ...
          // 自动搜索配图
          this.autoSearchImages(displayContent);
        },
      );
...
🎉 觉得有用别忘了三连支持一下！`;
  ...
  generateFormattedContentV2(content) {
    ...
    try {
      ...
      // 使用 platformFormatter.parseBody 递归解析 JSON
      // 这个方法可以处理多层嵌套的 JSON
      if (body.trim().startsWith('{') || body.trim().startsWith('[')) {
        console.log("🔧 generateFormattedContentV2 检测到 body 可能是 JSON，使用 parseBody 解析...");
        const parsedBody = platformFormatter.parseBody(body);
        if (parsedBody !== body) {
          body = parsedBody;
          console.log("🔧 generateFormattedContentV2 解析后 body 长度:", body.length, "开头:", body.substring(0, 50));
        }
      }
      ...
      // 降级处理 - 返回简单文本
      // 降级处理 - 返回简单文本
      const fallbackContent = this.normalizeContent(content);
      console.log("⚠️ 排版优化失败，使用降级处理:", {
        title: fallbackContent.title,
        contentLength: fallbackContent.content?.length || 0,
        contentType: typeof fallbackContent.content,
      });
      return {
  ...
  /**
   * 自动搜索配图和生成封面
   * 在内容生成后自动根据标题搜索相关配图并生成AI封面图
   */
  async autoSearchImages(content) {
    if (!content || !content.title) {
      console.log('无内容，跳过自动配图');
      return;
    }

    try {
      console.log('🔄 自动搜索配图，标题:', content.title);
      ...
      // 并行执行：搜索配图 + 生成封面图
      const [images, coverResult] = await Promise.allSettled([
        // 搜索相关配图
        imageService.searchImages(searchQuery, {
          platform,
          type: 'inline',
          count: 5,
        }),
        // AI生成封面图
        imageService.generateCoverImage(searchQuery, contentText, platform),
      ]);

      // 处理搜索到的配图
      if (images.status === 'fulfilled' && images.value && images.value.length > 0) {
        console.log('✅ 自动配图搜索成功，找到', images.value.length, '张图片');
        
        this.setData({
          searchedImages: images.value,
        ...
        if (!this.data.coverImage) {
          this.setData({
            coverImage: {
              url: images.value[0].url,
              title: searchQuery + ' - 封面',
            },
          });
          console.log('✅ 已设置搜索图片为封面');
        }
      } else {
        console.warn('⚠️ 配图搜索失败或无结果:', images.reason?.message);
      }

      // 处理AI生成的封面图（优先使用AI生成的）
      if (coverResult.status === 'fulfilled' && coverResult.value && coverResult.value.url) {
        console.log('✅ AI封面图生成成功:', coverResult.value.url);
        
        this.setData({
          coverImage: {
            url: coverResult.value.url,
            title: searchQuery + ' - AI封面',
            isAIGenerated: true,
          },
          aiCoverImage: coverResult.value,
        });
        console.log('✅ 已设置AI生成的封面图');
      } else {
        console.warn('⚠️ AI封面图生成失败:', coverResult.reason?.message);
      }

      // 将配图插入到HTML内容中
      this.insertImagesToContent();

    } catch (error) {
      console.warn('自动配图流程失败:', error);
      // 失败不影响主流程
    }
  },
  ...
  insertImagesToContent() {
    const { searchedImages, currentFormattedHtml, generatedContent } = this.data;
    
    if (!searchedImages || searchedImages.length === 0 || !currentFormattedHtml) {
      console.log('无图片或无HTML内容，跳过插入');
      return;
    }

    try {
      console.log('🖼️ 开始将图片插入到内容中...');
      
      // 使用 platformFormatter 的 generateWeChatHTML 方法重新生成带图片的HTML
      const title = generatedContent?.title || '';
      const body = generatedContent?.content || '';
      const tags = generatedContent?.tags || [];
      
      // 将搜索到的图片转换为 platformFormatter 需要的格式
      const formattedImages = searchedImages.slice(0, 3).map((img, index) => ({
        url: img.url,
        description: img.description || `${title} 相关配图 ${index + 1}`,
      }));

      // 重新生成带图片的HTML
      const htmlWithImages = platformFormatter.generateWeChatHTML(
        title,
        body,
        tags,
        true,  // highlight
      ...
      if (htmlWithImages && htmlWithImages !== currentFormattedHtml) {
        console.log('✅ 图片插入成功，HTML长度:', htmlWithImages.length);
        
        // 更新当前平台的HTML内容
        const updatedContents = { ...this.data.formattedContents };
        const currentPlatform = this.data.platformOptions[this.data.platformIndex];
        if (updatedContents[currentPlatform]) {
          updatedContents[currentPlatform].html = htmlWithImages;
          updatedContents[currentPlatform].preview = htmlWithImages;
        }

        this.setData({
          currentFormattedHtml: htmlWithImages,
          formattedContents: updatedContents,
        });
   ...
   * 搜索封面图片
   */
  async searchImagesForCover() {
    const { generatedContent } = this.data;
    const query = generatedContent?.title || '';
    ...
    try {
      ...
      if (images.length > 0) {
        ...
        wx.showToast({ title: '已设置封面', icon: 'success' });
      }
    } catch (error) {
   ...
   * 重新搜索图片（换一批）
   */
  async reSearchImages() {
    const { imageSearchQuery } = this.data;
    if (!imageSearchQuery) {
      wx.showToast({ title: '请先输入搜索词', icon: 'none' });
      return;
    }
    
    // 添加随机参数避免缓存
    const newQuery = imageSearchQuery + ' ' + Date.now();
    this.setData({ imageSearchQuery: newQuery });
    await this.searchImages();
    // 恢复原始搜索词
    this.setData({ imageSearchQuery: imageSearchQuery.split(' ')[0] });
  },
  ...
  selectImage(e) {
    ...
    // 预览图片
    const { searchedImages } = this.data;
    if (searchedImages && searchedImages[index]) {
      wx.previewImage({
        current: searchedImages[index].url,
        urls: searchedImages.map(img => img.url),
      });
    }
  },
  ...
  setAsCover(e) {
    const index = parseInt(e.currentTarget.dataset.index);
    const { searchedImages, generatedContent } = this.data;
    
    if (searchedImages && searchedImages[index]) {
      const img = searchedImages[index];
      this.setData({
        coverImage: {
          url: img.url,
          title: img.title || (generatedContent?.title || '') + ' - 封面',
          isAIGenerated: false,
        },
      });
  ...
  insertSingleImage(e) {
    ...
    if (!searchedImages || !searchedImages[index]) return;

    const img = searchedImages[index];
    console.log('插入单张图片:', img.url);

    // 构建图片HTML
    const imgHtml = `<div style="margin: 25px 0; text-align: center;">
    ...
    // 追加到当前HTML
    const currentHtml = this.data.currentFormattedHtml || '';
    ...
    const currentPlatform = this.data.platformOptions[this.data.platformIndex];
    if (updatedContents[currentPlatform]) {
      updatedContents[currentPlatform].html = newHtml;
      updatedContents[currentPlatform].preview = newHtml;
    }

    this.setData({
      currentFormattedHtml: newHtml,
      formattedContents: updatedContents,
    });
  ...
  insertAllImages() {
    ...
    if (!searchedImages || searchedImages.length === 0) {
      wx.showToast({ title: '没有可插入的图片', icon: 'none' });
      return;
    }

    // 调用 insertImagesToContent 方法
    this.insertImagesToContent();
  },
  ...
  onImageSearchInput(e) {
    this.setData({ imageSearchQuery: e.detail.value });
  },
  ...
  async publishArticle() {
    ...
    const tryPublish = async () => {
      try {
        const aiService = require('../../utils/ai-service.js');
        const result = await aiService.publishToWechat({
          title: articleJSON.title,
          content: articleJSON.content,
          cover_url: articleJSON.cover_url || ''
        });

        wx.hideLoading();

        if (result.success && result.data && (result.data.success === true || result.data.media_id)) {
          wx.showToast({ title: "推送成功", icon: "success" });
          return true;
        } else {
          throw new Error(result.data?.error || result.error || '推送失败');
        }
      } catch (error) {
        console.error(
...
请直接输出JSON，不要有任何额外说明。`;
        ...
        success: (res) => {
          ...
          if (res.statusCode !== 200) {
            ...
            if (res.statusCode === 429 && retryCount < maxRetries) {
              // 从响应中获取 Retry-After 头或使用默认等待时间
              const retryAfter = res.header?.['Retry-After']
                ? parseInt(res.header['Retry-After']) * 1000
                : 5000 * (retryCount + 1); // 默认递增：5s, 10s, 15s

              console.warn(
                `⏳ API 速率限制 (429)，${retryAfter / 1000}秒后自动重试... (${retryCount + 1}/${maxRetries})`,
              );
            ...
            // 其他非200状态码，构造友好的错误信息
            let errorMsg = `API返回状态码: ${res.statusCode}`;
            if (res.data?.error) {
              const errDetail = typeof res.data.error === 'string'
                ? res.data.error
          ...
          if (res.data && res.data.choices && res.data.choices.length > 0) {
            ...
            try {
              ...
              try {
                ...
                console.warn("二次解析也失败，尝试提取内容:", secondError.message);
  ...
  // 调用 MiniMax TokenPlanPlus API（优先级最高的模型）
  async callMiniMaxAPI(prompt, retryCount = 0) {
    const { apiKey, groupId, endpoint, model, temperature, top_p, max_tokens } = this.data.minimaxConfig;
    const maxRetries = 3;
    const timeout = 120000; // 120秒超时

    return new Promise((resolve, reject) => {
      console.log(`🚀 调用 MiniMax TokenPlanPlus API... (重试次数: ${retryCount}/${maxRetries})`);

      // 构建请求数据，body 中带 GroupId（TokenPlanPlus 鉴权方式）
      const requestData = {
        model: model,
        messages: [
          {
      ...
      // TokenPlanPlus 需要在 body 中带 group_id
      if (groupId) {
      ...
      wx.request({
        ...
        data: requestData,
        success: (res) => {
          console.log("✅ MiniMax API 调用成功, status:", res.statusCode);

          if (res.statusCode !== 200) {
            console.error("❌ MiniMax API状态码异常:", res.statusCode, res.data);
            ...
            if (res.statusCode === 429 && retryCount < maxRetries) {
              ...
              console.warn(
                `⏳ MiniMax API 速率限制 (429)，${retryAfter / 1000}秒后自动重试... (${retryCount + 1}/${maxRetries})`,
              );
              ...
              setTimeout(() => {
                this.callMiniMaxAPI(prompt, retryCount + 1)
                  .then(resolve)
                  .catch(reject);
              }, retryAfter);
              return;
            ...
            let errorMsg = `MiniMax API返回状态码: ${res.statusCode}`;
            if (res.data?.error) {
              const errDetail = typeof res.data.error === 'string'
                ? res.data.error
                : res.data.error.message || JSON.stringify(res.data.error);
          ...
          if (res.data && res.data.choices && res.data.choices.length > 0) {
            const rawContent = res.data.choices[0].message.content;
            console.log("📦 MiniMax 原始返回内容:", rawContent?.substring(0, 500));

            // 尝试解析JSON（与 callGLMAPI 相同的解析逻辑）
            try {
              let jsonStr = rawContent;
              const codeBlockMatch = rawContent.match(/```(?:json)?\s*([\s\S]*?)```/);
              if (codeBlockMatch && codeBlockMatch[1]) {
              ...
              const parsedContent = JSON.parse(jsonStr);
              console.log("✅ MiniMax JSON解析成功:", Object.keys(parsedContent));
              const normalizedContent = this.normalizeContent(parsedContent);
              resolve(normalizedContent);
            } catch (parseError) {
              console.warn(
                "⚠️ MiniMax JSON解析失败，尝试修复后解析:",
                parseError.message
              );
              ...
              try {
                ...
                const parsedContent = JSON.parse(fixedContent);
                console.log("修复后MiniMax JSON解析成功:", Object.keys(parsedContent));
                const normalizedContent = this.normalizeContent(parsedContent);
                resolve(normalizedContent);
              } catch (secondError) {
                console.warn("MiniMax二次解析也失败，尝试提取内容:", secondError.message);
            ...
            console.error("MiniMax API返回数据格式错误:", res.data);
            reject(new Error("MiniMax API返回数据格式错误"));
          }
        },
        fail: (error) => {
          ...
          if (
            ...
            console.log(
              `⏱️ MiniMax请求超时，准备重试... (${retryCount + 1}/${maxRetries})`,
            );
            setTimeout(
              () => {
                this.callMiniMaxAPI(prompt, retryCount + 1)
          ...
          reject(new Error("MiniMax API调用失败: " + error.errMsg));
        },
      });
    });
  },

  // 调用混元 Hunyuan API（通过云函数 agentAI，使用云开发内置AI能力）
  async callHunyuanAPI(prompt, retryCount = 0) {
    const { model } = this.data.hunyuanConfig || { model: "hunyuan-lite" };
    const maxRetries = 2;
    const timeout = 120000; // 120秒超时

    console.log(`🤖🤖🤖 [混元降级] 调用混元API... (模型: ${model}, 重试: ${retryCount}/${maxRetries}, prompt长度: ${prompt?.length || 0})`);
    console.log(`🤖🤖🤖 [混元降级] 云函数: agentAI, useZhipuAI: false`);

    return new Promise((resolve, reject) => {
      // 混元降级：直接调用智谱 GLM 作为最终降级方案（替代 agentAI 云函数）
      const aiService = require('../../utils/ai-service.js');
      aiService.callGLM(prompt, {
        agentType: 'content-creator',
        temperature: 0.7,
        max_tokens: 2000,
      }).then((result) => {
        console.log("✅ 降级模型调用成功:", result.success);

        if (result.success && result.reply) {
          const rawContent = result.reply;
          console.log("📦 降级模型返回内容长度:", rawContent.length);
          ...
          try {
            ...
            if (typeof rawContent === "string") {
              const codeBlockMatch = rawContent.match(/```(?:json)?\s*([\s\S]*?)```/);
              const jsonStr = codeBlockMatch ? codeBlockMatch[1].trim() : rawContent.trim();

              try {
                const parsed = JSON.parse(jsonStr);
                parsedContent = this.normalizeContent(parsed);
              } catch (parseErr) {
                console.warn("⚠️ 降级模型返回非JSON格式，包装为文本内容");
                parsedContent = {
                  title: this.extractTitleFromText(rawContent),
                  content: rawContent,
                  tags: [],
                  coverSuggestion: "请根据内容自行设计封面",
                  optimizationTips: ["内容由降级模型生成"],
                };
              }
            } else {
            ...
            console.error("❌ 降级模型结果解析失败:", error);
            reject(new Error("降级模型返回内容解析失败: " + error.message));
          }
        } else {
          console.error("❌ 降级模型返回数据格式异常:", result);
          reject(new Error("降级模型返回数据格式异常"));
        }
      }).catch((error) => {
        console.error("❌ 降级模型调用失败:", error);

        if (retryCount < maxRetries) {
          console.log(`⏱️ 降级模型请求失败，准备重试... (${retryCount + 1}/${maxRetries})`);
          setTimeout(() => {
            this.callHunyuanAPI(prompt, retryCount + 1)
              .then(resolve)
              .catch(reject);
          }, 3000 * (retryCount + 1));
          return;
        ...
        reject(new Error("降级模型调用失败: " + error.message));
      });
    });
  },

  // 从文本中提取标题（用于混元非JSON返回的兜底处理）
  extractTitleFromText(text) {
    if (!text || typeof text !== "string") return "AI生成的内容";
    // 尝试取第一行作为标题
    const firstLine = text.split("\n")[0].trim();
    // 去掉可能的 # 标记
    const title = firstLine.replace(/^#+\s*/, "").trim();
    return title.substring(0, 50) || "AI生成的内容";
  },
  ...
  normalizeContent(parsedContent) {
    ...
    // 使用 platformFormatter.parseBody 递归解析 JSON，并持续解析直到得到纯文本
    const extractTextContent = (data) => {
      let result = platformFormatter.parseBody(data);
      // 持续解析直到不再是 JSON
      let maxIter = 20;
      let iter = 0;
      while ((result.trim().startsWith('{') || result.trim().startsWith('[')) && iter < maxIter) {
        iter++;
        const nextResult = platformFormatter.parseBody(result);
    ...
    if (parsedContent.content && typeof parsedContent.content === "string") {
      ...
      if (contentStr.startsWith('{') || contentStr.startsWith('[')) {
        try {
          ...
          // 使用 parseBody 递归提取文本内容
          const extractedContent = extractTextContent(innerContent);
          
          if (innerContent.title && extractedContent) {
            console.log("🔧 成功提取嵌套内容，标题:", innerContent.title?.substring?.(0, 30));
            console.log("🔧 提取的内容长度:", extractedContent?.length, "开头:", extractedContent?.substring?.(0, 50));
            return {
              title: innerContent.title || parsedContent.title,
    ...
    if (parsedContent.title && typeof parsedContent.content === "string") {
      ...
      // 使用递归函数提取文本
      const extractedText = extractTextContent(contentText);
      console.log("🔧 extractTextContent 结果: 长度:", extractedText?.length, "开头:", extractedText?.substring?.(0, 50));
      
      if (extractedText !== contentText) {
        console.log("🔧 从 content 中递归提取到文本，长度:", extractedText.length);
    ...
    if (parsedContent.content && typeof parsedContent.content === "object") {
      ...
      // 使用递归提取函数处理其他情况
      const extractedText = extractTextContent(innerContent);
      console.log("🔧 [normalizeContent] 从对象中提取文本，长度:", extractedText.length);
  ...
  async saveToCloudHistory(historyItem) {
    try {
      ...
      // 保存到本地存储（替代云函数调用）
      const historyList = wx.getStorageSync('cloud_creation_history') || [];
      historyList.unshift(cloudData);
      if (historyList.length > 200) historyList.pop();
      wx.setStorageSync('cloud_creation_history', historyList);

      console.log("✅ 创作历史已保存到本地");
      return { success: true };
    } catch (error) {
  ...
  async saveToHistory(data) {
    ...
    try {
      ...
      try {
        // 构建保存到云端的数据
        // 使用 normalizeContent 确保内容格式正确
        const normalizedContent = this.normalizeContent(data);
        console.log("📦 云端保存前的内容处理:", {
          originalType: typeof data.content,
          normalizedType: typeof normalizedContent.content,
          normalizedLength: normalizedContent.content?.length || 0,
        });
        ...
        // 保存失败不影响本地保存
        console.warn("⚠️ 保存失败，本地已保存:", cloudError.message);
      }
    } catch (error) {
  ...
  async doPublishToWechat(publisher, currentAccount) {
    ...
    try {
      ...
      // 强制重新生成 HTML，确保内容不是 JSON 格式
      // 不使用 formattedContents，因为它可能在生成时包含 JSON
      if (generatedContent && generatedContent.content) {
        console.log('[发布] 强制重新生成 HTML，确保内容格式正确');
        ...
        // 提取正文内容 - 处理可能的 JSON 格式（支持嵌套JSON）
        let bodyContent = generatedContent.content;
        
        // 使用 platformFormatter 的 parseBody 方法直接解析
        // 这个方法已经被优化，可以处理多层嵌套的 JSON
        console.log('[发布] 原始 bodyContent 类型:', typeof bodyContent, '长度:', typeof bodyContent === 'string' ? bodyContent.length : 'N/A');
        console.log('[发布] 原始 bodyContent 前200字符:', typeof bodyContent === 'string' ? bodyContent.substring(0, 200) : JSON.stringify(bodyContent).substring(0, 200));
        
        // 直接使用 platformFormatter.parseBody 解析
        bodyContent = platformFormatter.parseBody(bodyContent);
        
        // 持续解析直到不再是 JSON
        let maxIterations = 20;
        let iteration = 0;
        while ((bodyContent.trim().startsWith('{') || bodyContent.trim().startsWith('[')) && iteration < maxIterations) {
          iteration++;
          console.log(`[发布] 第 ${iteration} 次解析，bodyContent 仍是JSON格式`);
          const nextContent = platformFormatter.parseBody(bodyContent);
          if (nextContent === bodyContent) {
            console.log('[发布] 解析结果相同，停止迭代');
            break;
          }
          bodyContent = nextContent;
        }
        
        console.log('[发布] 最终 bodyContent 类型:', typeof bodyContent, '长度:', bodyContent?.length);
        console.log('[发布] 最终 bodyContent 前100字符:', bodyContent?.substring(0, 100));
        ...
        console.log('[发布] HTML 生成成功，长度:', htmlContent.length);
      } else if (formattedContents && formattedContents["微信公众号"]) {
        // 降级：使用 formattedContents（不推荐，作为备用）
        htmlContent = formattedContents["微信公众号"].html || "";
        console.log('[发布] 降级使用 formattedContents HTML, 长度:', htmlContent.length);
      }

      // 如果没有内容，报错
      if (!htmlContent) {
        throw new Error('无法生成文章内容，请重新生成内容后重试');
      }
      ...
      // 特定错误提示
      if (errorMessage.includes("-504003") || errorMessage.includes("FUNCTIONS_TIME_LIMIT") || errorMessage.includes("timed out") || errorMessage.includes("云函数执行超时")) {
        errorMessage = "发布操作超时\n\n可能原因：\n1. 网络波动导致响应慢\n2. 后端服务正在处理中\n\n建议：\n1. 稍后检查微信公众号草稿箱，确认文章是否已成功发布\n2. 如果草稿箱没有文章，请重试发布\n3. 如果反复超时，请检查后端服务状态";
      } else if (errorMessage.includes("40164") || errorMessage.includes("IP") || errorMessage.includes("whitelist")) {
        errorMessage =
          "IP地址不在白名单中\n\n解决方法：\n1. 登录微信公众号后台\n2. 进入【设置与开发】→【基本配置】\n3. 在【IP白名单】中添加：\n   39.108.254.228\n4. 管理员扫码确认后生效\n\n每个公众号都需要单独设置IP白名单";
      } else if (
    ...
    try {
      ...
      if (!finalCoverUrl) {
        ...
        try {
          ...
          console.warn('封面图生成失败，使用默认图:', error);
  ...
  postToDraftAPI(articleJSON) {
    ...
    // 直接通过 wx.request 调用服务器API（替代 social-media-proxy 云函数）
    console.log('通过服务器API发布草稿');

    return new Promise((resolve, reject) => {
      const aiService = require('../../utils/ai-service.js');
      aiService.publishToWechat({
        title: articleJSON.title,
        content: articleJSON.content,
        cover_url: articleJSON.cover_url || ''
      }).then((result) => {
        console.log('服务器API发布响应:', result);
        ...
        if (result.success) {
          ...
          if (apiData && (apiData.success === true || apiData.media_id)) {
            ...
            resolve({
              success: false,
              error: apiData?.error || '发布失败'
            });
          }
        } else {
          resolve({
            success: false,
            error: result.error || '发布失败，请稍后重试'
          });
        }
      }).catch((err) => {
        console.error('发布调用失败:', err);
        resolve({
          success: false,
          error: err.message || '发布失败，请检查网络'
        });
      });
    });
  },
