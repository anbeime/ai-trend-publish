          ...
          try {
            ...
            for (let j = 0; j < storyboardImages.length; j++) {
              ...
              try {
                ...
                // 生成视频（使用百炼/通义万相，支持参考图）
                const videoResult =
                  await pageContext.generateVideo(videoPrompt, {
                    reference_urls: [imageUrl], // 传入参考图片
                    duration: 5, // 视频时长 5 秒
                  });
            ...
            // 为脚本生成配音和字幕
            if (videoResults.filter(v => v.videoUrl).length > 0) {
              try {
                this.addMessage(pageContext, "system", "🎙️ 正在生成配音和字幕...");
                
                // 获取脚本内容
                const scriptContent = agentOutputs.scriptWriter?.reply || "";
                
                if (scriptContent && pageContext.generateVideoAssets) {
                  const assetsResult = await pageContext.generateVideoAssets(scriptContent, {
                    voice: "longxiaochun", // 默认使用温柔女声
                    subtitleStyle: {
                      position: "bottom",
                      fontSize: 22,
                      fontColor: "#FFFFFF",
                  ...
                  if (assetsResult.success) {
                    // 将配音和字幕信息添加到结果中
                    agentOutputs.storyboard.voiceover = assetsResult.voiceover;
                    agentOutputs.storyboard.subtitleData = assetsResult.subtitles;
                    pageContext.setData({ agentOutputs });
                    
                    this.addMessage(
                      pageContext,
                      "system", 
                      ` 配音和字幕生成完成：${assetsResult.voiceover.narrations?.length || 0} 段配音`
                    );
                  }
                }
              } catch (assetsError) {
                console.error("生成配音字幕失败:", assetsError);
                this.addMessage(
                  pageContext,
                  "system",
                  "⚠️ 配音或字幕生成失败：" + assetsError.message,
                );
              }
            }
