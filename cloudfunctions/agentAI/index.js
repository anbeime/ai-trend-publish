...
// 智谱AI API配置 - 优先从环境变量读取，提高安全性
const ZHIPU_API_CONFIG = {
  endpoint: 'https://open.bigmodel.cn/api/paas/v4/chat/completions',
  apiKey: process.env.GLM_API_KEY || '' // 从环境变量读取API密钥
}

// 配置检查
if (!ZHIPU_API_CONFIG.apiKey) {
...
- 对比指标: [指标1]、[指标2]、[指标3]`
    ...
    if (useZhipuAI) {
      ...
      const model = event.model || 'glm-4-flash'
      
      try {
        const https = require('https');
        const url = new URL(ZHIPU_API_CONFIG.endpoint);
