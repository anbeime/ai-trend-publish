...
Page({
  ...
  async loadHotspots(showToast = true) {
    ...
    try {
      ...
      // 使用 Promise.race 实现超时控制（云函数已配置为60秒超时）
      ...
      // 50秒超时（云函数60秒超时，留10秒余量）
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('请求超时')), 50000);
      });
  ...
  applyFilters() {
    ...
    hotspots = hotspots.map((item, index) => {
      ...
      // 确保分类在有效列表中
      const validCategories = ['科技', '生活', '娱乐', '美食', '旅行', '财经', '游戏', '体育'];
  ...
  // 获取备用热点数据（仅在数据获取失败时使用）
  getMockHotspots() {
    // 返回提示数据，而不是假的热点内容
    return [
      {
        title: '暂无热点数据',
        description: '热点数据加载失败，请点击刷新按钮重试，或检查网络连接',
        source: '系统提示',
        hotScore: 0,
        fitScore: 0,
        category: '生活',
