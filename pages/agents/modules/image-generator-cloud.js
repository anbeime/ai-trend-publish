...
class ImageGeneratorCloud {
   ...
   */
  async generateImageGPTImage2(prompt, options = {}) {
    const {
      size = '1024x1024',
    ...
    console.log('通过云函数调用 GPT-Image-2 生图:', {
      prompt: prompt.substring(0, 100),
      width,
      height,
      model,
    ...
    try {
      ...
      console.log('GPT-Image-2 云函数生图结果:', result.result);

      if (result.result && result.result.success) {
        console.log('GPT-Image-2 生图成功，URL:', result.result.imageUrl);
        return result.result.imageUrl;
      } else {
        throw new Error(result.result?.message || 'GPT-Image-2 生图失败');
      }
    } catch (error) {
      console.error('GPT-Image-2 云函数生图失败:', error);
      throw error;
    }
  }

  // 兼容层：统一的图片生成入口
  // 优先使用 GPT-Image-2，降级到混元，再降级到GLM
  async generateImage(prompt, options = {}) {
    ...
    try {
      ...
      console.warn('GPT-Image-2 不可用，尝试降级:', gptError.message);
    }
